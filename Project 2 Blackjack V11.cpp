/*
    ...
    Date: Febuary 14, 2026 2:57 PM
    Purpose: Project 2: Blackjack
    Version 11
    Version Notes: added comments and fixed game stopping after instant blackjack 
...
void rule(bool rulesho = true);//shows player rules
void playgam(string pName, float &bal, int &winP, int &lossP, int &gameNum, int &totlDra, char results[], bool deck[][13]);//betting enabled
void playgam(string pName, int &winP, int &lossP, int &gameNum, int &totlDra, char results[], bool deck[][13]);//betting disabled
...
int main(int argv,char **argc){
    ...
    cout<<"Welcome to Blackjack!"<<endl;//welcome user
    cout<<endl<<"What is your name?"<<endl;//ask for name
    cin>>pName;

    ifstream inFile("Blackjack_Data.txt");//open txt file

    if(!inFile){
    cout<<"Error opening file! Closing program."<<endl;
    exit(EXIT_FAILURE);//exit program if error opening file
    }

    vector<string> players;
    while(inFile>>fName){//look at player names in txt file to see if inputted name matches
    ...
    if(match){//welcome back message if user inputted name matches name in txt file
        cout<<endl<<"Welcome back, "<<pName<<"!"<<endl;
    }

    rule(rulesho = true);//show rules to user
    

    cout<<"Start game of blackjack? Y/N"<<endl;//ask user to start
    cin>>play;

    while(play=='Y' || play == 'y'){//while player says yes to more games

        for(int row = 0; row < 4; row++){//new deck every game
            for(int col = 0; col < 13; col++){
                deck[row][col] = false;
            }
        }

        cout<<endl<<"Do you want to allow betting for this game? Y/N"<<endl;//ask if user wants betting
        cout<<pName<<"'s Balance: $"<<fixed<<setprecision(2)<<bal<<endl;//show users balance
        cin>>betAlow;

        betting = (betAlow == 'y' || betAlow == 'Y') ? true : false;//set betting as on or off

        if(betting == true){
            playgam(pName, bal, winP, lossP, gameNum, totlDra, results, deck);//game with betting
        }else if(betting == false){
            playgam(pName, winP, lossP, gameNum, totlDra, results, deck);//game without betting
        }
        ...
        if(bal > 1000000.00){
            cout<<"Balance Overflow Warning!";//overflow check
        }
    ...
    cout<<endl<<"There were "<<totlDra<<" draws this session"<<endl;//tell user about cards drawn

    netChan = static_cast<float>(bal - 1000.00);//set net change to balance
    ...
    cout<<endl<<"Sorted Session History:"<<endl;//sort game history
    for(int i = 0; i < gameNum; i++){
        cout<<"Game "<<(i + 1)<<": "<<results[i]<<endl;
    }

    cout<<endl<<"Session History"<<endl;//game history
    for(int i = 0; i < gameNum; i++){
        cout<<"Game "<<(i + 1)<<": "<<results[i]<<endl;
    }

    cout<<"Storage size of results: "<<sizeof(results)<<" bytes"<<endl;//tell user storge size of results
...
void playgam(string pName, float &bal, int &winP, int &lossP, int &gameNum, int &totlDra, char results[], bool deck[][13]){//games with bets
    //Declare Varibles
        /*
        C++ Varible Guidelines:
        7 characters or less
    ...
    gameNum++;//increase game counter
        ...
        while(totalP < 21 && !pStand){//if no instant blackjacks or busts
            if(stopgam==false){
                ...
                if(stdOrhi=='h' || stdOrhi=='H'){//if player hits
                    ...
                    totalP += cVal;
                    pCarNum++;//increase number of players cards
                    cout<<endl<<pName<<"'s Card "<<pCarNum<<": "<<cName<<endl;//show card
                    cout<<pName<<" has: "<<totalP<<endl;//show player total
        ...
        while(totalD < 17){//dealer stands on 17
            if(stopgam == false){
                ...
                totalD += cVal;
                dCarNum++;//increase number of dealers cards
                cout<<endl<<"Dealer Card "<<dCarNum<<": "<<cName<<endl;//show card
...
//overloaded function
void playgam(string pName, int &winP, int &lossP, int &gameNum, int &totlDra, char results[], bool deck[][13]){//games without bets
    //Declare Varibles
        /*
        C++ Varible Guidelines:
        7 characters or less
...
int drawC(string &cardNam, int &totlDra, bool deck[][13]){//drawing cards
    int rank, suitNum;
    ...
    string ranks[] = {"Ace", "2", "3", "4", "5", "6", "7", "8", "9", "10", "Jack", "Queen", "King"};//card ranks
...
bool findP(const vector<string> &v, string name){//linear search
    bool found = false;
...
void bublSrt(vector<string> &a){//buble sort
    int n = a.size();
    for(int i = 0; i < n - 1; i++){
...
void selcSrt(char a[], int n){//selection sort 0829
    for(int i = 0; i < n - 1; i++){
        int minIdx = i;
        for(int j = i + 1; j < n; j++){
            if(a[j] < a[minIdx]){
